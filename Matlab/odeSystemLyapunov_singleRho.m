function dy=odeSystemLyapunov_singleRho(~,y,p)
   ...
   dy(9)= p.r2*(1-s)*(1+g)-p.rho*p.r1*s*c;
   ...
   Jac(1,5)=p.rho/(g + 1) + (n*p.nu*(g/p.betaG + 1))/(p.betaC*(c/p.betaC + 1)^2);
   Jac(1,6)=- p.rho*c/(g + 1)^2 - (n*p.nu)/(p.betaG*(c/p.betaC + 1));
   Jac(2,1)=(p.nu*(g/p.betaG + 1))/(c/p.betaC + 1);
   Jac(2,2)=-p.gammaA*p.rho - p.phi*(m1 + m2*p.phi2);
   Jac(2,3)=-p.phi*a;
   Jac(2,4)=-p.phi*p.phi2*a;
   Jac(2,5)=-(n*p.nu*(g/p.betaG + 1))/(p.betaC*(c/p.betaC + 1)^2);
   Jac(2,6)=(n*p.nu)/(p.betaG*(c/p.betaC + 1));
   Jac(3,2)=-p.rho*p.km1*p.phi*m1;
   Jac(3,3)=-p.gammaM*(p.gammaM2*m2 + 1) - a*p.rho*p.km1*p.phi;
   Jac(3,4)=p.km2 - p.gammaM*p.gammaM2*m1;
   Jac(3,5)=p.rho;
   Jac(4,2)=p.rho*p.km1*p.phi*m1;
   Jac(4,3)=a*p.rho*p.km1*p.phi;
   Jac(4,4)=-p.gammaM-p.km2;
   Jac(5,1)=p.rho*p.kn*((2*n)/(p.betaN^2 + n^2) - (2*n^3)/(p.betaN^2 + n^2)^2);
   Jac(5,2)=p.rho*p.gammaA*((2*a)/(a^2 + p.betaA^2) - (2*a^3)/(a^2 + p.betaA^2)^2);
   Jac(5,3)=p.km;
   Jac(5,5)=-1;
   Jac(5,8)=1;
   Jac(6,4)=p.rho*p.kg;
   ...
   Jac(9,5)=-p.rho*p.r1*s;
   Jac(9,6)=-p.r2*(s-1);
   Jac(9,9)=- c*p.rho*p.r1 - p.r2*(g + 1);
   dy=dy';
